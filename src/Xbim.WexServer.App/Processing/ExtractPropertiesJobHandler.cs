...
/// Stores properties as a SQLite artifact file and optionally persists to Xbim database.
/// </summary>
public class ExtractPropertiesJobHandler : IJobHandler<ExtractPropertiesJobPayload>
{
    ...
    public async Task HandleAsync(string jobId, ExtractPropertiesJobPayload payload, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                // Optionally persist to Xbim database
                if (payload.PersistToDatabase)
                {
                    ...
                    await PersistToXbimDatabaseAsync(tempSqlitePath, payload.ModelVersionId, cancellationToken);
    ...
    private async Task PersistToXbimDatabaseAsync(
        string sqlitePath,
        Guid modelVersionId,
        CancellationToken cancellationToken)
    {
        ...
        // Read from SQLite and persist to Xbim DB
        ...
        var elementIdMap = new Dictionary<string, Guid>(); // SQLite ID -> Xbim ID
