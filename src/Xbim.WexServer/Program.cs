...
using Xbim.WexServer.Endpoints;
using Xbim.WexServer.HealthChecks;
...
builder.Services.Configure<Xbim.WexServer.Auth.OAuthTokenOptions>(
    builder.Configuration.GetSection("OAuth"));
builder.Services.AddSingleton<Xbim.WexServer.Auth.IOAuthTokenService, Xbim.WexServer.Auth.OAuthTokenService>();
...
app.UseExceptionHandler(exceptionHandlerApp =>
{
    exceptionHandlerApp.Run(async context =>
    {
        ...
        if (exception is Xbim.WexServer.Auth.InsufficientScopeException scopeException)
        {
        ...
        else if (exception is Xbim.WexServer.Auth.WorkspaceIsolationException isolationException)
        {
            context.Response.StatusCode = StatusCodes.Status403Forbidden;
            context.Response.ContentType = "application/json";
            await context.Response.WriteAsJsonAsync(new
        ...
        else if (exception is Xbim.WexServer.Auth.ForbiddenAccessException)
        {
            context.Response.StatusCode = StatusCodes.Status403Forbidden;
            context.Response.ContentType = "application/json";
            await context.Response.WriteAsJsonAsync(new
